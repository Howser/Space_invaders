...
public class Weapon implements Serializable {
	...
	public Weapon(int amount, float speed, float spread, int cooldown,
			int damage, Sprite shotSprite, int dir) {
	...
	public ShotEntity[] fire(float x, float y) {
		if (canFire) {
			...
			for (int i = 0; i < shots.length; i++) {
				...
				double angle = (projectileSpread / shots.length * (i + 0.5))
						+ dir * 180 - (projectileSpread / 2);
				float xSpeed = projectileSpeed
						* (float) Math.sin(Math.toRadians(angle));
				float ySpeed = projectileSpeed
